...
## 7. Phase 3 고도화 범위 (2026-02-15)

```
목표:
  홈페이지를 모던하게 재구성하고 탐색 효율/운영 품질을 함께 개선

범위:
  - 홈 IA 재설계 (Hero/Featured/Latest)
  - 카테고리 레일 + Featured 큐레이션
  - 검색 UX 강화 (가중치, 키보드 탐색, 최근 검색)
  - 목록 필터/정렬 + 쿼리 상태 복원
  - 성능 최적화 (LCP/CLS/JS 로드 전략)
  - 운영 품질 게이트 스크립트
...
```
- GitHub Pages 제약 준수 (기존 플러그인 범위 내)
- 기존 permalink, baseurl, relative_url 규칙 유지
- 기능 추가 시 기본 동작(포스트 목록/페이지네이션) 회귀 금지
```
